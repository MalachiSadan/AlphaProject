...
func used to remove problematic columns so the PCA will end successfully
```{r}
...
prepare top 10 abundant sequences in every sample
```{r}
...
prepare top 1000 abundant sequences in tumor samples
```{r}
tumor <- repLoad("~/Desktop/TCR -tumor")
pub_t <- pubRep(tumor$data, .col = "aa", "prop")
```

include only these sequences, and only if they appear in PBMC samples too
```{r}
