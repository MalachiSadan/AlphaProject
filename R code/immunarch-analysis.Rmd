...
#vis(clontop, .by = "Tissue Source", .meta = lung$meta)
#clontopstage <- repClonality(sub_lc, "top")
#vis(clontopstage, .by = "Disease Stage", .meta = lung$meta)
...
```{r}
ggplot(f1_res, aes(x=Model,y=f1.score,fill=Matrix)) + geom_bar(stat = "identity", position = "dodge") + ggtitle("ML Classification algorithms evaluation with F1 score") + labs(x= "ML model", y= "F1 Score") + theme_bw()
```

```{r}
ggplot(f1_res[f1_res$Matrix == "Threshold",], aes(x=Matrix,y=f1.score, fill=Model)) + 
  geom_bar(stat = "identity", position = "dodge") + scale_fill_brewer(palette = "YlGn") + theme_bw() + labs(y="F1 score", x="Threshold matrix") + theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
```
